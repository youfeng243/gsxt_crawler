...
from common.util import judge_feature
...
class GsxtBaseWorker(TaskBaseWorker):
    ...
    def cracker_search_list_content(self, keyword, item=None, session=None):
        ...
        json_data, content = self.get_captcha_geetest_full(url, '#searchText', '#click', keyword,
                                                           '#searchtips', origin_session=session)
        if content is None:
            return None

        if judge_feature(content):
            self.log.error("滑动验证码识别为验证码拦截页面: province = {} keyword = {}".format(self.province, keyword))
            return None
    ...
    def get_search_list_content(self, keyword, session):
        ...
        if cookie is None:
            self.log.info('获得cookie信息为None: province = {province} keyword = {keyword}'.format(
                province=self.province, keyword=keyword))
            return self.cracker_search_list_content(keyword, item=item, session=session)
        ...
        if r.text.find('验证码不正确') != -1:
            self.log.warn('cookie 已过时: province = {} company = {}'.format(self.province, keyword))
            return self.cracker_search_list_content(keyword, item=item, session=session)

        # 判断是否是验证码拦截页面
        if judge_feature(r.text):
            self.log.error("通过cookie获得验证码拦截页面, 需要滑动验证码识别: province = {} keyword = {}".format(self.province, keyword))
            return self.cracker_search_list_content(keyword, item=item, session=session)

        self.log.info('通过cookie获得数据: province = {} company = {}'.format(self.province, keyword))
        return r.text
    ...
    def save_search_list(self, company, code, param_list):
        ...
        for param in param_list:
            ...
            if priority == 0:
                ...
                seed = {
                    '_id': util.generator_id({}, search_name, self.province),
        ...
        self.log.info("扩展链接数目为: province = {} company = {} {}".format(
            self.province, company, len(data_list)))
            ...
            if origin_session is not None:
                ...
                if isinstance(cookies, list) and len(cookies) > 0:
                    for cookie_dict in cookies:
                        ...
                        # origin_session.cookies.update(cookie)
                        try:
                            origin_session.cookies[cookie_dict['name']] = cookie_dict['value']
                        except Exception as e:
                            self.log.error("设置cookie错误:")
                            self.log.exception(e)
    ...
    def get_captcha_geetest_full(self, url, input_selector, search, keyword, result, success=None,
        ...
        proxy = self.get_http_proxy()['http']
        self.log.info("当前滑动验证码使用代理: {}".format(proxy))
    ...
    def query_offline_company(self, company_name, seed_code, query_name):
        try:
            ...
            # 存储搜索列表基本信息
            data_list, match_param = self.save_search_list(company_name, seed_code, param_list)
            if len(data_list) > 0 and match_param is None:
