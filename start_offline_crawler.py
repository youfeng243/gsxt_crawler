...
class StartTaskCrawler(object):
    ...
    def init_worker(self, config_list):
        for key, value in config_list.iteritems():
            ...
            self.db_init_count_dict[key] = 1

            self.log.info('初始化 {key} 完成..'.format(key=key))

    # 重新初始化db
    def init_db_iter(self):
        for key, value in self.config_list.iteritems():
            if key not in self.db_iter_valid_dict or not self.db_iter_valid_dict[key]:
                source_select_param = eval(value['source_select_param'])
                self.db_iter_dict[key] = self.__get_iterator_company_list(self.source_table, source_select_param)
                self.db_iter_valid_dict[key] = True

                if key in self.db_init_count_dict:
                    self.db_init_count_dict[key] += 1
                else:
                    self.db_init_count_dict[key] = 1
                self.log.info('当前省份重新初始化db, 目前初始化次数为: {} {}'.format(key, self.db_init_count_dict[key]))
        ...
        for province, db_iter in self.db_iter_dict.iteritems():
            ...
            try:
                ...
                if item is None:
                    ...
                    self.log.error("当前省份遍历数据得到为None: province = {}".format(province))
                    continue
...
def main():
    config = 'config/offline_gsxt_all.conf'
    source_table = "offline_all_list"
    province = None
